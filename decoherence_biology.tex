...
\begin{abstract}
Decoherence drives the quantum-to-classical transition, but its information-geometric structure---the precise dimensional and metric changes it imposes on quantum state space---has not been quantified in the Bures/Fisher--Rao framework. We show that decoherence is a dimensional collapse: the dephasing channel maps a $(d^2{-}1)$-dimensional quantum manifold onto the $(d{-}1)$-dimensional classical probability simplex, annihilating the $d^2{-}d$ coherence directions while preserving the $d{-}1$ population dimensions. At diagonal states, the classical and quantum tangent subspaces are Bures-orthogonal. We compute the exact Bures principal angle $\theta_B$ between these subspaces at every full-rank qubit state, obtaining $\cos^2\theta_B = r_z^2 r_\perp^2/[(1{-}r_\perp^2)(1{-}r_z^2)]$, where $r_z$ is the population imbalance and $r_\perp$ the coherence magnitude. For $d \geq 3$, Bures-orthogonality fails whenever a nonzero coherence has a decoupled spectator index, and a perturbative formula quantifies the departure. We apply this framework to the Fenna--Matthews--Olson photosynthetic complex ($d = 7$), where decoherence collapses 48 quantum dimensions to 6 classical dimensions. Computing the six Bures principal angles as a function of dephasing strength, we find that the quantum-classical transition is rapid: at physiological dephasing rates ($\gamma \sim 50$--$200$\,cm$^{-1}$), all principal angles exceed $89^\circ$ and the system is geometrically nearly classical, with strong quantum-classical mixing confined to much weaker dephasing ($\gamma \lesssim 10$\,cm$^{-1}$).
\end{abstract}
...
\item[\emph{(Geometry of the split.)}] We compute the exact Bures principal angle $\theta_B$ between the classical and quantum tangent subspaces at every full-rank qubit state (Theorem~\ref{thm:bures-angle}). For $d \geq 3$, Bures-orthogonality fails whenever a nonzero coherence has a decoupled spectator index (Theorem~\ref{thm:spectator-orthogonality}), and a perturbative formula quantifies the departure (Theorem~\ref{thm:qudit-angles}).
\item[\emph{(Application.)}] We apply the framework to the Fenna--Matthews--Olson (FMO) photosynthetic complex, where $d = 7$ sites yield a collapse from 48 to 6 information-geometric dimensions. Computing the six Bures principal angles as a function of dephasing strength, we find that at physiological dephasing rates the system is geometrically nearly classical, with the strong quantum-classical mixing regime lying at much weaker dephasing.
\end{enumerate}
...
\end{equation}
where the first term generates unitary evolution (which creates and rotates coherences, and redistributes populations) and the second term dephases coherences at rate $\gamma$. In the pure-dephasing limit ($H = 0$), this channel preserves populations and drives off-diagonal elements $\rho_{kl} \to e^{-\gamma t}\rho_{kl}$; the full dynamics~\eqref{eq:lindblad} is a competition between coherent Hamiltonian evolution and incoherent dephasing.

\subsection{Geometry of the quantum-classical split in FMO}
\label{sec:fmo-geometry}

Theorems~\ref{thm:qudit-angles} and~\ref{thm:spectator-orthogonality} establish that for $d \geq 3$, the Bures-orthogonal decomposition $T_\rho^{\mathrm{diag}} \perp_{g_B} T_\rho^{\mathrm{off}}$ fails whenever $\rho$ carries nonzero off-diagonal entries. For the FMO complex ($d = 7$), the diagonal and off-diagonal tangent subspaces have dimensions $d - 1 = 6$ and $d^2 - d = 42$ respectively, and their relative orientation is described by $d - 1 = 6$ principal angles $\theta_1 \leq \cdots \leq \theta_6$.

We compute these principal angles numerically at each dephasing rate $\gamma$ by evolving~\eqref{eq:lindblad} from an initial excitation at site~1 for 5\,ps, then computing the Bures-metric cross-Gram matrix between diagonal and off-diagonal basis directions and extracting singular values (see Appendix). Figure~\ref{fig:fmo-collapse} shows the results.

At weak dephasing ($\gamma \lesssim 1$\,cm$^{-1}$), the minimum principal angle drops below $50^\circ$: the classical population subspace and the quantum coherence subspace are far from orthogonal, meaning that population parameters and coherence parameters are strongly mixed in the Fisher information sense. At moderate dephasing ($\gamma \sim 5$--$20$\,cm$^{-1}$), all angles are in the range $70^\circ$--$85^\circ$: the quantum-classical split is approaching but has not yet reached orthogonality. At strong dephasing ($\gamma > 50$\,cm$^{-1}$), all six angles are within a few degrees of $90^\circ$ and the decomposition is effectively classical.

\subsection{ENAQT and the classical limit}
\label{sec:fmo-enaqt}

ENAQT \cite{rebentrost2009environment,plenio2008dephasing} occurs because pure quantum evolution can trap excitations in interference patterns, while moderate dephasing breaks destructive interference and opens transport channels. The ENAQT optimal dephasing rate for FMO models with a reaction-centre sink is $\gamma_{\mathrm{opt}} \sim 50$--$200$\,cm$^{-1}$ depending on model details \cite{rebentrost2009environment,cao2020quantum}.

Our principal angle analysis places this regime in geometric context. The physiological dephasing range ($\gamma \sim 50$--$200$\,cm$^{-1}$, corresponding to decoherence times ${\sim}\,50$--$200$\,fs at 300\,K) falls in the region where all principal angles exceed $89^\circ$ (Figure~\ref{fig:fmo-collapse}). That is, FMO at physiological conditions is geometrically close to classical: the quantum-classical split is nearly orthogonal. The biologically relevant quantum effects arise from the \emph{small residual departure} from classical orthogonality, not from a regime of strong quantum-classical mixing. The most geometrically interesting transition---where angles pass through $45^\circ$--$80^\circ$---occurs at dephasing rates ($\gamma \sim 1$--$20$\,cm$^{-1}$) well below the physiological range.

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{fig4_fmo_collapse.pdf}
\caption{\textbf{Bures principal angles in the FMO complex.} The six principal angles $\theta_1, \ldots, \theta_6$ between the diagonal (classical) and off-diagonal (quantum) tangent subspaces of the Bures metric, plotted as a function of Haken--Strobl dephasing rate $\gamma$, for the FMO state after 5\,ps of evolution from site~1 excitation. The shaded band spans the minimum to maximum angle. At weak dephasing the subspaces are far from orthogonal (strong quantum-classical mixing); at physiological dephasing rates (shaded region, $\gamma \sim 50$--$200$\,cm$^{-1}$) all angles exceed $89^\circ$ and the system is geometrically nearly classical.}
...
We have shown that decoherence is a dimensional collapse in the information-geometric sense: $d^2 - 1 \to d - 1$ dimensions, with the Bures metric restricting to Fisher--Rao on the surviving simplex. The exact Bures separation angle quantifies the geometry of the classical-quantum split at every full-rank qubit state, and the perturbative theorem and spectator criterion extend the analysis to arbitrary dimension. Applied to the FMO complex, principal angle analysis reveals that at physiological dephasing rates the system is geometrically close to classical (all principal angles within ${\sim}\,1^\circ$ of $90^\circ$), while the strong quantum-classical mixing regime lies at much weaker dephasing ($\gamma \lesssim 10$\,cm$^{-1}$).

\subsection{Limitations}

The qubit separation angle (Theorem~\ref{thm:bures-angle}) is exact; for $d \geq 3$, results are partial (spectator theorem + perturbative expansion + conjecture). The Haken--Strobl dephasing model used for FMO is simplified: the real protein-solvent bath has structured spectral density, non-Markovian dynamics, and spatially correlated fluctuations \cite{cao2020quantum}. Our model also omits the reaction-centre sink that is essential for defining ENAQT transport efficiency. Extending the information-geometric analysis to structured baths and to models with explicit trapping is an open problem.

\subsection{Outlook: from quantum to classical coherence}
\label{sec:bridge}

A suggestive observation emerges from Theorem~\ref{thm:metric-restriction}: the Fisher--Rao metric that governs statistical inference on the surviving classical simplex \emph{is} the restriction of the Bures metric from the quantum state manifold. In biological systems, decoherence completes on femtosecond timescales, after which dynamics unfold on the classical simplex. The same information-geometric tools---participation ratio, effective dimensionality, coherence time---apply to both quantum and classical dynamics.

This raises the question of whether biological coherence on longer timescales \cite{todd2026coherence,todd2026intelligence} can be understood as a second stage of dimensional evolution: dissipative dynamics within the Fisher--Rao simplex further shaping the effective dimensionality of accessible states. A rigorous connection would require showing how the information geometry of the quantum-classical transition maps onto effective descriptions of classical biological dynamics. We leave this as a program for future work, noting only that the geometric continuity established by Theorem~\ref{thm:metric-restriction} makes such a connection structurally natural.
...
The Lindblad master equation~\eqref{eq:lindblad} is integrated numerically using a fourth-order Runge--Kutta scheme with time step $\Delta t = 1.0$\,fs. The FMO Hamiltonian~\eqref{eq:fmo-hamiltonian} uses site energies and couplings from Adolphs and Renger \cite{adolphs2006proteins}. For each dephasing rate $\gamma$, the density matrix is evolved from the initial state $\rho(0) = |1\rangle\langle 1|$ (excitation at site~1) for $t = 5$\,ps, which is sufficient for coherences to reach a quasi-stationary balance between Hamiltonian-driven creation and dephasing-driven decay. (For dephasing rates $\gamma \geq 5$\,cm$^{-1}$, the coherence decay time $\gamma^{-1} \lesssim 1$\,ps is shorter than the evolution time.) The state is regularized to full rank ($\rho \mapsto \rho + 10^{-10}\,I/d$) before computing the six Bures principal angles between the diagonal and off-diagonal tangent subspaces.
