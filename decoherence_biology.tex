...
% Title information
\title{Information Geometry of the Quantum-Classical Transition in Biological Systems}
\author{Ian Todd\\
...
\begin{abstract}
Decoherence drives the quantum-to-classical transition, but its information-geometric structure---the precise dimensional and metric changes it imposes on quantum state space---has not been quantified in the Bures/Fisher--Rao framework. We show that decoherence is a dimensional collapse: the dephasing channel maps a $(d^2{-}1)$-dimensional quantum manifold onto the $(d{-}1)$-dimensional classical probability simplex, annihilating the $d^2{-}d$ coherence directions while preserving the $d{-}1$ population dimensions. At diagonal states, the classical and quantum tangent subspaces are Bures-orthogonal. We compute the exact Bures principal angle $\theta_B$ between these subspaces at every full-rank qubit state, obtaining $\cos^2\theta_B = r_z^2 r_\perp^2/[(1{-}r_\perp^2)(1{-}r_z^2)]$, where $r_z$ is the population imbalance and $r_\perp$ the coherence magnitude. For $d \geq 3$, Bures-orthogonality fails whenever a nonzero coherence has a decoupled spectator index. We apply this framework to the Fenna--Matthews--Olson photosynthetic complex ($d = 7$), where decoherence collapses 48 quantum dimensions to 6 classical dimensions. The spectator theorem applies directly to the FMO coupling structure, and the Bures angle quantifies the intermediate quantum-classical regime exploited by environment-assisted quantum transport. After quantum decoherence completes (${\sim}\mathrm{fs}$), the surviving Fisher--Rao simplex becomes the arena for classical biological coherence on millisecond-to-second timescales---two stages of dimensional collapse connected by the same geometric framework.
\end{abstract}
...
\item[\emph{(Geometry of the split.)}] We compute the exact Bures principal angle $\theta_B$ between the classical and quantum tangent subspaces at every full-rank qubit state (Theorem~\ref{thm:bures-angle}). For $d \geq 3$, Bures-orthogonality fails whenever a nonzero coherence has a decoupled spectator index (Theorem~\ref{thm:spectator-orthogonality}).
\item[\emph{(Application.)}] We apply the framework to the Fenna--Matthews--Olson (FMO) photosynthetic complex, where $d = 7$ sites yield a collapse from 48 to 6 information-geometric dimensions. The spectator theorem applies directly to the FMO coupling structure, and the Bures angle quantifies the intermediate regime exploited by ENAQT.
\item[\emph{(Bridge.)}] After quantum decoherence completes on femtosecond timescales, biological systems maintain classical coherence on millisecond-to-second timescales \cite{todd2026coherence}. The Fisher--Rao geometry that emerges from quantum dimensional collapse is the same geometry that governs classical biological coherence---two stages of dimensional collapse connected by a single mathematical framework.
\end{enumerate}

The paper is organized as follows. Section~\ref{sec:geometry} establishes the information-geometric framework. Section~\ref{sec:decoherence} contains the core mathematical results. Section~\ref{sec:thermodynamics} derives the thermodynamic cost of dimensional collapse. Section~\ref{sec:fmo} applies the framework to the FMO complex. Section~\ref{sec:bridge} connects quantum decoherence to classical biological coherence. Section~\ref{sec:discussion} discusses implications and limitations.
...
For $d$-dimensional systems, there are $d - 1$ principal angles between $T_\rho^{\mathrm{diag}}$ and $T_\rho^{\mathrm{off}}$.
...
\begin{enumerate}
\item[\emph{(i)}] If $C = 0$, all $d - 1$ principal angles equal $\pi/2$ (Theorem~\ref{thm:orthogonal-decomposition}).
\item[\emph{(ii)}] Near the simplex ($\|C\|$ small), the cross-Gram matrix has entries
\begin{equation}
\label{eq:cross-gram}
...
\item[\emph{(iii)}] For $d \geq 3$: if $\|C\|$ is small and $C \neq 0$, at least one principal angle is strictly less than $\pi/2$.
...
\begin{proof}
\emph{Part~(ii):} Expand $L_a = L_a^{(0)} + L_a^{(1)} + O(\|C\|^2)$. At zeroth order, $(L_a^{(0)})_{kk} = x_a^k/p_k$ with zero off-diagonal entries. At first order, the Lyapunov equation gives $(L_a^{(1)})_{mn} = -C_{mn}(x_a^m/p_m + x_a^n/p_n)/(p_m + p_n)$ for $m \neq n$. For $R_{kl}$, the first-order diagonal corrections are $(L_R^{(1)})_{kk} = -2\mathrm{Re}[C_{kl}]/[p_k(p_k + p_l)]$. Summing the three first-order contributions to $g_B(D_a, R_{kl})$ yields~\eqref{eq:cross-gram}.

\emph{Part~(iii):} For $d \geq 3$ with $C_{kl} \neq 0$, choose $D_a$ with $x_a^k = 1$, $x_a^m = -1$ for some $m \notin \{k, l\}$, zeros elsewhere. Then $x_a^k/p_k + x_a^l/p_l = 1/p_k \neq 0$, giving nonzero cross-Gram. For $d = 2$, the unique traceless direction $\mathrm{diag}(1,-1)$ satisfies $1/p_1 + (-1)/p_2 = 0$ at equal populations, allowing orthogonality; the exact criterion is Theorem~\ref{thm:bures-angle}.

\emph{Part~(iv):} Under $C \mapsto (1{-}2\gamma)C$, equation~\eqref{eq:cross-gram} scales as $(1{-}2\gamma)$ and vanishes as $\gamma \to 1/2$.
\end{proof}

\begin{theorem}[Spectator orthogonality criterion for $d \geq 3$]
\label{thm:spectator-orthogonality}
Let $d \geq 3$ and let $\rho$ be a full-rank state with $\rho_{st} \neq 0$ for some $s \neq t$. If there exists a spectator index $r \notin \{s,t\}$ such that $\rho_{rs} = \rho_{rt} = 0$, then the Bures-orthogonal decomposition $T_\rho^{\mathrm{diag}} \perp_{g_B} T_\rho^{\mathrm{off}}$ fails.
\end{theorem}

\begin{proof}
For any diagonal $X_D$ and off-diagonal $X_O$, the SLD metric identity gives
\begin{equation}
\label{eq:sld-identity}
g_B(X_D, X_O) = \tfrac{1}{2}\sum_k x_k\, (L_{X_O})_{kk},
\end{equation}
so orthogonality requires every off-diagonal SLD to have a flat diagonal.

Since $\rho_{rs} = \rho_{rt} = 0$, the $(r,s)$ and $(r,t)$ entries of the Lyapunov equation for perturbations on the $(s,t)$ block form a homogeneous system with positive determinant, giving $(L_{X_O})_{rs} = (L_{X_O})_{rt} = 0$ and hence $(L_{X_O})_{rr} = 0$.

For the diagonal entry at $s$:
\begin{equation}
\label{eq:Lss-formula}
(L_{X_O})_{ss} = -\mathrm{Re}[\rho_{st}(L_{X_O})_{ts}]/p_s.
\end{equation}
The $(s,t)$ entry of the Lyapunov equation yields a scalar equation for $(L_{X_O})_{st}$ whose solution satisfies $(L_{X_O})_{st} \neq 0$. Writing $\rho_{st} = |\rho_{st}|e^{i\alpha}$:
\begin{align}
\label{eq:Rformula}
\mathrm{Re}[\rho_{st}(L_{R_{st}})_{ts}] &= \frac{2|\rho_{st}|\cos\alpha}{(p_s + p_t)(1 - |\rho_{st}|^2/(p_s p_t))}, \\
\label{eq:Iformula}
\mathrm{Re}[\rho_{st}(L_{I_{st}})_{ts}] &= \frac{2|\rho_{st}|\sin\alpha}{(p_s + p_t)(1 - |\rho_{st}|^2/(p_s p_t))}.
\end{align}
Since $|\rho_{st}| > 0$, at least one of these is nonzero, making $(L_{X_O})_{ss} \neq 0$ while $(L_{X_O})_{rr} = 0$: the SLD diagonal is not flat, so orthogonality fails.
\end{proof}
...
Theorem~\ref{thm:spectator-orthogonality} proves this conjecture whenever the coherence graph contains an edge whose endpoints share a common non-neighbor. The conjecture extends to fully-coherent states (complete coherence graph). Two rigorous results support the extension: Theorem~\ref{thm:qudit-angles}(iii) proves it near the simplex for all $d$, and a numerical survey of random states per dimension ($d = 3, 5, 7$, Figure~\ref{fig:qudit-angles}) finds no non-diagonal state achieving orthogonality.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{fig6_qudit_angles.pdf}
\caption{\textbf{Numerical survey of qudit principal angles.}  For each dimension $d \in \{3, 5, 7\}$, random full-rank density matrices are drawn from the Hilbert--Schmidt measure (blue histogram), and the minimum principal angle $\min_a \theta_a$ between the diagonal and off-diagonal tangent subspaces is computed via the Bures metric. The dashed red line marks $90^\circ$ (perfect orthogonality). No state approaches orthogonality; the distribution shifts further from $90^\circ$ as $d$ increases, supporting Conjecture~\ref{conj:global-orthogonality}. The $d = 7$ panel corresponds to the FMO Hilbert space dimension.}
...
Under progressive Haken--Strobl dephasing \cite{haken1973exactly} at rate $\gamma$, the density matrix evolves according to the Lindblad master equation
\begin{equation}
\label{eq:lindblad}
\dot{\rho} = -\frac{i}{\hbar}[H_{\mathrm{FMO}}, \rho] + \gamma \sum_{k=1}^7 \left(|k\rangle\langle k| \rho |k\rangle\langle k| - \frac{1}{2}\{|k\rangle\langle k|, \rho\}\right),
\end{equation}
which drives coherences to zero at rate $\gamma$ while preserving populations. The effective quantum dimensionality $D_{\mathrm{eff}}^Q$ traces a trajectory from near 48 (at $\gamma = 0$) toward 6 (at large $\gamma$), with the exact path depending on the initial state and Hamiltonian structure.

\subsection{Spectator theorem applied to FMO}
\label{sec:fmo-spectator}

The coupling structure of $H_{\mathrm{FMO}}$ determines which site pairs have negligible coupling ($|J_{kl}| < 5$\,cm$^{-1}$). Defining the coherence graph by significant couplings, several pairs have spectators:

\begin{itemize}
\item \textbf{Sites 1--2} ($J_{12} = -87.7$\,cm$^{-1}$): Site~5 is a spectator---coupled to site~1 ($J_{15} = 6.7$\,cm$^{-1}$) but not to site~2 ($J_{25} = 0.7$\,cm$^{-1}$). By Theorem~\ref{thm:spectator-orthogonality}, Bures-orthogonality fails for any state retaining $(1,2)$ coherence.
\item \textbf{Sites 3--4} ($J_{34} = -53.5$\,cm$^{-1}$): Site~5 is a spectator---coupled to site~4 ($J_{45} = -70.7$\,cm$^{-1}$) but not to site~3 ($J_{35} = -2.2$\,cm$^{-1}$).
\item \textbf{Sites 2--4} ($J_{24} = 8.2$\,cm$^{-1}$): Site~5 is a spectator---coupled to site~4 ($J_{45} = -70.7$\,cm$^{-1}$) but not to site~2 ($J_{25} = 0.7$\,cm$^{-1}$).
\end{itemize}

The spectator theorem guarantees that during the transport window---when coherences between these site pairs are nonzero---the quantum-classical split is geometrically ``messy'': the classical population subspace and the quantum coherence subspace are not orthogonal. This is precisely the regime where ENAQT operates.

\subsection{The Bures angle during noise-assisted transport}
\label{sec:fmo-enaqt}

ENAQT \cite{rebentrost2009environment,plenio2008dephasing} occurs because pure quantum evolution can trap excitations in interference patterns, while moderate dephasing breaks destructive interference and opens transport channels. At zero dephasing, the system explores the full 48-dimensional quantum state space but may be localized; at infinite dephasing, it is confined to the 6-dimensional classical simplex and transport proceeds by incoherent hopping. The optimum lies between these limits, at a dephasing rate of approximately $\gamma_{\mathrm{opt}} \sim 195$\,cm$^{-1}$ for the FMO complex \cite{rebentrost2009environment}.

The Bures angle provides a geometric characterization of this intermediate regime. At each dephasing rate $\gamma$, the density matrix $\rho(t; \gamma)$ has a well-defined set of principal angles between the diagonal and off-diagonal tangent subspaces. At $\gamma = 0$, coherences are maximal and the principal angles can be small (strong mixing between classical and quantum sectors). At $\gamma \to \infty$, all coherences vanish and all angles reach $\pi/2$ (perfect orthogonality). The ENAQT optimal dephasing rate corresponds to an intermediate Bures angle regime: the system exploits partial dimensional collapse, maintaining enough coherence for quantum speedup while allowing enough dephasing to avoid localization.

Figure~\ref{fig:fmo-collapse} shows the effective dimensionality of the FMO system as a function of dephasing strength, with the ENAQT optimal rate marked.

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{fig4_fmo_collapse.pdf}
\caption{\textbf{Dimensional collapse in the FMO complex.} Normalized coherence magnitude (Frobenius norm of off-diagonal elements) as a function of Haken--Strobl dephasing rate $\gamma$ for the FMO complex, with initial excitation at site~1 evolved for 5\,ps. At weak dephasing ($\gamma \lesssim 5$\,cm$^{-1}$), the state explores the 48-dimensional quantum state space with significant coherence. At the ENAQT optimal dephasing rate ($\gamma_{\mathrm{opt}} \approx 195$\,cm$^{-1}$, vertical dashed line), coherences are largely suppressed; at strong dephasing, the state is confined to the 6-dimensional classical simplex.}
\label{fig:fmo-collapse}
\end{figure}

\subsection{FMO coherence graph}
\label{sec:fmo-graph}

The coupling structure of the FMO Hamiltonian can be represented as a graph where vertices are chromophore sites and edges connect pairs with significant electronic coupling ($|J_{kl}| > 5$\,cm$^{-1}$). Figure~\ref{fig:fmo-graph} shows this graph with spectator pairs highlighted.

\begin{figure}[H]
\centering
\includegraphics[width=0.6\textwidth]{fig2_fmo_graph.pdf}
\caption{\textbf{FMO coherence graph and spectator structure.} Vertices represent the seven BChl sites; edges connect pairs with significant coupling ($|J_{kl}| > 5$\,cm$^{-1}$). Edge widths are proportional to coupling strength. Red dashed lines indicate edges where site~5 acts as a spectator (coupled to one endpoint but not the other), guaranteeing failure of Bures-orthogonality by Theorem~\ref{thm:spectator-orthogonality}.}
\label{fig:fmo-graph}
\end{figure}

%% ============================================================
\section{From Quantum to Classical Coherence}
\label{sec:bridge}

\subsection{Two stages of dimensional collapse}
\label{sec:two-stages}

The dimensional collapse framework reveals a two-stage hierarchy in biological systems:

\textbf{Stage 1: Quantum decoherence} (${\sim}\mathrm{fs}$). The dephasing channel maps $\mathcal{M}_Q$ (dimension $d^2 - 1$) onto $\Delta_{d-1}$ (dimension $d - 1$), annihilating coherences. In the FMO complex, this destroys 42 of 48 information-geometric dimensions in ${\sim}\,50$\,fs. The Bures metric restricts to the Fisher--Rao metric on the surviving classical simplex (Theorem~\ref{thm:metric-restriction}).

\textbf{Stage 2: Classical dissipation} ($\mathrm{ms}$--$\mathrm{s}$). Within the surviving Fisher--Rao simplex, dissipative dynamics further reduce the effective dimensionality of accessible states. Biological systems maintain coherent classical dynamics---coordinated oscillatory activity in neural populations, metabolic feedback loops, immune cascades---against entropic pressure on timescales of milliseconds to seconds.

The geometry is continuous across these stages: the Fisher--Rao metric that governs classical biological dynamics \emph{is} the Bures metric restricted to the classical subspace. The same mathematical tools---participation ratio, effective dimensionality, coherence time---apply on both sides of the quantum-classical divide.

\subsection{Classical coherence time as a rate limit}
\label{sec:coherence-time}

After quantum decoherence completes, biological computation occurs on the classical simplex. The rate at which a biological oscillator assembly can register distinct states is bounded by the coherence time $\tau_{\mathrm{coh}}$---the timescale over which coupled oscillators maintain phase relationships sufficient to define a reproducible state \cite{todd2026coherence}. This bound dominates all other timescale limits (quantum decoherence, thermodynamic, power dissipation) by orders of magnitude: quantum decoherence in biological systems completes in femtoseconds, while classical coherence times range from milliseconds (neural oscillations) to seconds (circadian rhythms).

The $d - 1$ dimensions surviving quantum decoherence are the arena for this classical dynamics. In a system with $N$ coupled biological oscillators, the effective classical dimensionality can be large ($D_{\mathrm{eff}} \sim N$ for weakly coupled oscillators, lower for strongly synchronized assemblies), and the coherence time $\tau_{\mathrm{coh}}$ bounds the rate of state registration within this space.

\subsection{Intelligence as high-dimensional coherence}
\label{sec:intelligence}

The observable dimensionality bound $D_{\mathrm{crit}} = C_{\mathrm{commit}} \cdot \tau_e / (\alpha \cdot h_\varepsilon)$ constrains the maximum effective dimensionality that a biological system can maintain against decoherence pressure \cite{todd2026intelligence}. This bound operates entirely within the classical regime---on the Fisher--Rao simplex that emerges after quantum dimensional collapse.

The two-stage picture clarifies the relationship between quantum and classical coherence in biological systems. Quantum coherence contributes to biological function primarily through Stage~1 processes (ENAQT, radical pair mechanisms) that complete on ultrafast timescales. Classical coherence---the maintenance of high-dimensional coordinated dynamics against dissipation---is the dominant mode of biological information processing, operating on timescales $10^{12}$ times longer than quantum decoherence.

``Measurement is dimensional collapse'' holds at both levels: quantum measurement collapses $d^2 - 1$ dimensions to $d - 1$ (this paper), while classical observation of a biological system collapses its effective dimensionality further through coarse-graining. The quantum-to-classical transition is the first---and fastest---stage of a hierarchy of dimensional collapses that biological systems navigate.
...
We have shown that decoherence is a dimensional collapse in the information-geometric sense: $d^2 - 1 \to d - 1$ dimensions, with the Bures metric restricting to Fisher--Rao on the surviving simplex. The exact Bures separation angle quantifies the geometry of the classical-quantum split at every qubit state, and the spectator theorem extends to arbitrary dimension. Applied to the FMO complex, this framework reveals that ENAQT operates in the intermediate regime of partial dimensional collapse---the system exploits the geometric structure of the quantum-classical transition itself.

\subsection{Limitations}

The qubit separation angle (Theorem~\ref{thm:bures-angle}) is exact; for $d \geq 3$, results are partial (spectator theorem + perturbative expansion + conjecture). The Haken--Strobl dephasing model used for FMO is simplified: the real protein-solvent bath has structured spectral density, non-Markovian dynamics, and spatially correlated fluctuations \cite{cao2020quantum}. Extending the information-geometric analysis to structured baths is an open problem.

The two-stage collapse picture (Section~\ref{sec:bridge}) is conceptual rather than formally derived: a rigorous connection would require showing how Fisher--Rao geometry on the quantum-classical simplex maps onto effective information geometry of classical biological dynamics. This is a program for future work.

\subsection{Open questions}

\begin{enumerate}
\item \emph{Conjecture~\ref{conj:global-orthogonality}}: proving that for $d \geq 3$, only diagonal states achieve Bures-orthogonality.
...
The Lindblad master equation~\eqref{eq:lindblad} is integrated numerically using a fourth-order Runge--Kutta scheme with time step $\Delta t = 0.1$\,fs. The FMO Hamiltonian~\eqref{eq:fmo-hamiltonian} uses site energies and couplings from Adolphs and Renger \cite{adolphs2006proteins}. For each dephasing rate $\gamma$, the steady-state density matrix is obtained by evolving from the initial state $\rho(0) = |1\rangle\langle 1|$ (excitation at site~1) for $t = 10$\,ps. The QFIM is computed at the evolved state using the SLD procedure above.
