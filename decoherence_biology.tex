...
For the geometry analysis, we also consider the dephasing-only model ($\Gamma_{\mathrm{trap}} = 0$), which preserves total population and allows clean comparison of states across dephasing rates. In the pure-dephasing limit ($H = 0$, $\Gamma_{\mathrm{trap}} = 0$), off-diagonal elements decay as $\rho_{kl} \to e^{-\gamma t}\rho_{kl}$; the full dynamics~\eqref{eq:lindblad} is a competition between coherent Hamiltonian evolution, incoherent dephasing, and trapping.
...
\caption{\textbf{Transport efficiency and Bures geometry in the FMO complex.} \textbf{(a)}~Transport efficiency $\eta$ as a function of Haken--Strobl dephasing rate $\gamma$, with trapping at site~3 ($\Gamma_{\mathrm{trap}} = 1$\,ps$^{-1}$). Both natural entry points (site~1, site~6) show a clear ENAQT peak at $\gamma_{\mathrm{opt}} \approx 100$--$150$\,cm$^{-1}$. \textbf{(b)}~Minimum Bures principal angle $\theta_{\min}$ between the diagonal and off-diagonal tangent subspaces, computed from the dephasing-only model evolved for 5\,ps. Dotted vertical lines mark the efficiency-peak $\gamma$ from panel~(a). At the ENAQT optimum, all principal angles exceed $89^\circ$: optimal transport occurs in a regime that is geometrically close to classical.}
...
The qubit separation angle (Theorem~\ref{thm:bures-angle}) is exact; for $d \geq 3$, results are partial (spectator theorem + perturbative expansion + conjecture). The Haken--Strobl dephasing model used for FMO is simplified: the real protein-solvent bath has structured spectral density, non-Markovian dynamics, and spatially correlated fluctuations \cite{cao2020quantum}. The transport efficiency is computed with a phenomenological sink ($\Gamma_{\mathrm{trap}} = 1$\,ps$^{-1}$), while the principal angle analysis uses dephasing-only dynamics to avoid conflating geometric structure with population loss. Extending the information-geometric analysis to structured baths and non-Markovian dephasing is an open problem.
