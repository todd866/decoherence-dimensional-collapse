...
\item \textbf{But at molecular scales:} Ion channels ($d \sim 4$) and protein-folding microdomains ($d \sim 6$) operate at $\gamma_{\mathrm{bio}}/J_{\max} \sim O(1$--$30)$---the same regime as FMO/PE545.  The paper defines a non-classicality index $\chi$ and coordinated dimensionality $D_{\mathrm{coord}}$ to formalize this: deep classical oscillations ($\chi_{\mathrm{osc}} \approx 0$) reliably coordinate $N$ non-classical molecular sites ($\chi_m > 0$), so representational dimensionality scales with $N$, not oscillation frequency.
\item \textbf{Extensive payload condition:} High dimensionality does not by itself imply non-classicality.  The aggregate non-classical payload becomes extensive only if a nonzero density of molecular modules has $\chi_i>0$ and redundancy stays bounded.  Under those assumptions, low-frequency carriers support larger payload because they entrain larger sets.
\end{itemize}

\begin{figure}[ht]
\centering
\includegraphics[width=0.92\textwidth]{fig8_neural_comparison.pdf}
\caption{Cross-scale comparison.  Photosynthetic complexes and illustrative molecular models (ion channel, protein mid-fold, chromatin) sit near the quantum-classical transition; neural oscillations are ${\sim}12$ orders of magnitude past it.  The detection threshold (right) shows how much microenvironment shielding would be needed.}
\end{figure}
...
Three validated systems spanning $12$ orders of magnitude in dephasing, plus three illustrative molecular models, all follow the same geometric framework: \textbf{photosynthetic and molecular-scale systems sit near the quantum-classical boundary; neural oscillator assemblies are deep classical at oscillation-frequency scales but coordinate molecular systems that remain non-classical ($\chi_m>0$).}

The paper formalizes this as a \textbf{multi-scale architecture}: a non-classicality index $\chi$ measures departure from the classical simplex, and coordinated dimensionality scales with entrained site count $N$, not oscillation frequency.  The new extensive-payload argument makes the key condition explicit: large systems matter only if a nonzero fraction of molecular modules remain near-boundary and their redundancy is bounded.

An illustrative ion-channel model ($d = 4$; Appendix~C) gives $\theta_{\min}\approx 88.5^\circ$ and $\chi\approx2.8\times10^{-4}$ at biological dephasing, while an illustrative protein mid-fold model ($d=6$; Appendix~C) gives $\theta_{\min}\approx88.2^\circ$ and $\chi\approx3.7\times10^{-4}$.  Both are non-classical but near-classical.  An additional chromatin model ($d = 8$; Appendix~C) sits near the boundary at reference parameters ($\theta_{\min}\approx87^\circ$) but is sensitivity-limited at weak coupling, motivating future work with experimentally constrained Hamiltonians.
...
See the molecular payload case studies & Appendix~C (ion channel, protein mid-fold, chromatin) \\
...
\item Appendix~C molecular payload models (if interested in molecular genericity)
