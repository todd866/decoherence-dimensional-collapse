...
\item \textbf{Effect-size split:} Absolute uplift scales as $\Delta D_{\mathrm{coord}} = N\chi_m(d_m^2-d_m)$, while relative uplift is fixed at $R_{\mathrm{coord}}=1+\chi_m d_m$ (Fig.~9). For ion-channel and protein-mid-fold anchors, the per-site effect is small but accumulates with large $N$: expensive to retain in simulation, not impossible.
\end{itemize}
