...
An illustrative molecular-oscillator model (chromatin, $d = 8$; Appendix~C) explores whether this near-boundary position extends beyond photosynthesis.  At reference parameters ($\gamma_{\mathrm{bio}}/J_{\max} = 8$), $\theta_{\min} \approx 87^\circ$---suggestively near the boundary.  However, a sensitivity sweep reveals that the position is sensitive to coupling strength, motivating future work with experimentally determined Hamiltonians.
