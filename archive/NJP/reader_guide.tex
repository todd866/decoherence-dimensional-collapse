...
\hypersetup{
    ...
    linkcolor=blue,
    urlcolor=blue,
    pdftitle={A Reader's Guide to Information Geometry of the Quantum-Classical Transition},
    pdfauthor={Ian Todd}
}

\title{\textbf{A Reader's Guide to\\Information Geometry of the Quantum-Classical\\Transition: From Photosynthetic Excitons to Neural Oscillations}}
\author{Ian Todd}
\date{Companion note for the manuscript submitted to \emph{New Journal of Physics}}
...
\item \textbf{Geometry at bio dephasing:} All 9 principal angles are $90.00^\circ$ to machine precision.  Deep classical.
\item \textbf{Detection threshold:} ${\sim}12$ orders of magnitude of microenvironment shielding would be needed for any quantum effect to become geometrically distinguishable.
\end{itemize}

\begin{figure}[ht]
\centering
\includegraphics[width=0.92\textwidth]{fig8_neural_comparison.pdf}
\caption{Cross-scale comparison.  Photosynthetic complexes sit near the quantum-classical transition; neural oscillations are ${\sim}12$ orders of magnitude past it.  The detection threshold (right) shows how much microenvironment shielding would be needed.}
\end{figure}
...
Three biological systems spanning $12$ orders of magnitude in dephasing all follow the same geometric framework: \textbf{photosynthetic complexes sit near the quantum-classical boundary; neural oscillator assemblies are geometrically indistinguishable from the classical simplex.}

This does not rule out quantum effects in neural microenvironments (ion channels, protein interiors) at molecular energy scales.  It means the framework converts ``is it quantum?'' from a yes/no question into a quantitative geometric bound.
...
See the cross-scale comparison & \S6.4 + Table in Appendix~C \\
...
See the neural parameters & Appendix~C (microcircuit models) \\
\bottomrule
\end{tabular}
\end{table}
