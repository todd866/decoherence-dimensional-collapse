...
% Title information
\title{Information Geometry of the Quantum-Classical Transition:\\ From Photosynthetic Excitons to Neural Oscillations}
\author{Ian Todd\\
...
Working memory in primate cortex relies on high-dimensional mixed selectivity: individual neurons respond to conjunctions of task variables, and the population-level representation lives on a high-dimensional manifold maintained by gamma ($30$--$70$\,Hz) and beta ($15$--$30$\,Hz) oscillatory bursts \cite{lundqvist2016gamma,rigotti2013importance,fusi2016neurons,miller2018working}. Pinotsis and Miller \cite{pinotsis2023cytoelectric} have shown that stable electric field dynamics emerge from these oscillatory interactions, suggesting that the relevant dynamical degrees of freedom are the coupled oscillator populations of cortical columns rather than individual neurons.

A cortical column can be modelled as $d \sim 10$--$100$ coupled oscillator populations spanning the gamma band \cite{buzsaki2012mechanisms}. The question is: where does such an assembly sit on the quantum-classical spectrum measured by the principal angle framework?
...
At conservative thermal dephasing ($T = 310$\,K), the biological dephasing rate for neural oscillations is $\gamma_{\mathrm{bio}} = k_BT/\hbar \approx 4 \times 10^{13}$\,Hz, giving $\gamma_{\mathrm{bio}}/J_{\max} \approx 8 \times 10^{12}$. At this ratio, all nine principal angles are $90.00^\circ$ to machine precision: the system is geometrically indistinguishable from the classical simplex.

We define a \emph{detection threshold} at $\theta_{\min} = 85^\circ$: below this angle, quantum-classical mixing is at least as strong as FMO at its biological operating point. The detection threshold occurs at $\gamma/J_{\max} \sim O(1)$, the same dimensionless ratio as in photosynthetic systems. To reach this regime from biological dephasing, the effective dephasing rate would need to be reduced by ${\sim}12$ orders of magnitude from bulk thermal estimates (Figure~\ref{fig:neural-comparison}b). For comparison, photosynthetic protein baths provide ${\sim}2$--$3$ orders of magnitude of shielding from bulk thermal dephasing \cite{cao2020quantum}---insufficient by ${\sim}9$ orders for neural oscillation frequencies.

This does not rule out quantum effects in shielded microenvironments (ion channel pores, protein interiors, microtubule lumens) that operate at molecular energy scales rather than oscillation frequencies. The framework is agnostic: it quantifies the detection threshold for any claimed quantum effect as the dephasing reduction needed to bring principal angles below $90^\circ$. Tegmark \cite{tegmark2000importance} reached a similar conclusion via decoherence timescale estimates; ours is a complementary geometry argument that provides a continuous bound rather than a binary timescale comparison.

\subsection{The geometric spectrum across biological scales}
\label{sec:cross-scale}

Figure~\ref{fig:neural-comparison} places the three systems on a single axis of dimensionless dephasing $\gamma/J_{\max}$:
\begin{itemize}
...
\item \textbf{Neural gamma} ($d = 10$): $\gamma_{\mathrm{bio}}/J_{\max} \approx 8 \times 10^{12}$. Angles pinned at $90.00^\circ$. Deep classical.
\end{itemize}

These three systems span $12$ orders of magnitude in $\gamma_{\mathrm{bio}}/J_{\max}$, yet all exhibit the same ENAQT-equivalent geometry at $\gamma/J_{\max} \sim O(1)$, confirming the scale invariance of the principal angle framework.

The result connects to the classical coherence program: neural computation operates \emph{within} the Fisher--Rao simplex, exploiting classical coherence---oscillatory phase relationships, high-dimensional population dynamics---rather than quantum coherence \cite{todd2026coherence,todd2026intelligence}. The principal angle framework converts the qualitative claim ``neural systems are classical'' into a quantitative geometric bound: the minimum microenvironment shielding factor needed for any quantum effect to become geometrically distinguishable.

As an empirical validation, we repeat the analysis with the Potjans--Diesmann canonical cortical microcircuit model ($d = 8$, empirical connectivity from \cite{potjans2014cell}; see Appendix~\ref{app:neural}). Both the synthetic ($d = 10$) and empirical ($d = 8$) neural models give the same qualitative result: all angles $\approx 90^\circ$ at biological dephasing, with the ENAQT-equivalent regime at $\gamma/J_{\max} \sim O(1)$.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{fig8_neural_comparison.pdf}
\caption{\textbf{Cross-scale quantum-classical boundary.} \textbf{(a)}~Minimum Bures principal angle $\theta_{\min}$ as a function of dimensionless dephasing $\gamma/J_{\max}$ for FMO ($d = 7$, blue), PE545 ($d = 8$, orange), and a cortical gamma-oscillation assembly ($d = 10$, green). Dashed vertical lines mark biological dephasing for FMO and PE545; the neural biological dephasing ($\gamma_{\mathrm{bio}}/J_{\max} \sim 10^{13}$) lies far off-scale to the right. All three systems exhibit the same transition geometry at $\gamma/J_{\max} \sim O(1)$, confirming scale invariance. \textbf{(b)}~Detection threshold for the neural system: $\theta_{\min}$ vs.\ hypothetical dephasing, with the biological dephasing marked (dashed red). The top axis shows the microenvironment shielding factor (ratio of biological to effective dephasing) that would be needed to enter the geometrically interesting regime. Approximately $12$ orders of magnitude of shielding are required---far beyond what photosynthetic protein baths provide (${\sim}2$--$3$ orders).}
\label{fig:neural-comparison}
\end{figure}
...
\begin{table}[H]
\centering
\caption{\textbf{Cross-scale comparison.} Optimal dimensionless dephasing $\gamma_{\mathrm{opt}}/J_{\max}$, minimum principal angle at the transport optimum $\theta_{\min}^{\mathrm{opt}}$, and minimum principal angle at biological dephasing $\theta_{\min}^{\mathrm{bio}}$ for all four systems. At biological dephasing, both neural models give $\theta_{\min} = 90.00^\circ$ to machine precision.}
...
Neural (synthetic)  & 10 & $99 \to 9$  & ${\sim}\,O(1)$      & $89.8^\circ$           & $90.00^\circ$ \\
Neural (PD empirical)   &  8 & $63 \to 7$  & ${\sim}\,O(1)$      & $89.5^\circ$           & $90.00^\circ$ \\
\hline
\end{tabular}
\end{table}
